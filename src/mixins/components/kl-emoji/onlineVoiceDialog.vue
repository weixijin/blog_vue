<template>
    <kl-dialog width="300px" center :header="false" :footer="false" :dialogVisible.sync="visable">
        <div class="flex-column-wrap p-20 flex-center-wrap pr p-t-40" @click.stop="() => {}">
            ...
            <!-- 录制时长 -->
            <div class="m-t-20">通话时长：{{ getAudioTime }}</div>

            <el-button type="danger" size="small" class="m-t-20" @click.stop="stop">挂断</el-button>
        </div>
    </kl-dialog>
</template>
...
export default {
    ...
    computed: {
        ...
        getAudioTime() {
            return (this.audioTime / 1000).toFixed(1) + 's'
        },
    },
    watch: {
        value(val) {
